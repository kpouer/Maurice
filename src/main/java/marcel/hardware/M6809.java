...
public class M6809 {
    private static final int sound_size = 1024;
    private final Memory mem;

    // Sound emulation parameters
    private final byte[] sound_buffer;
    ...
    public byte getSound_buffer(int index) {
        return sound_buffer[index];
    }
    ...
    private int INDEXE() {
        ...
        switch (m) {
            ...
            case 0x88:  //i_d_8_X;
                ...
                return M;
            case 0x89:  //i_d_16_X;
                m2 = (mem.read(PC) << 8) | mem.read(PC + 1);
                PC += 2;
    ...
    private int LD16(int M, int c) {
        int R = (((mem.read(M)) << 8) | mem.read(M + 1)) & 0xFFFF;
        m1 = ovfl;
        sign = R >> 8;
        res = (res & 0x100) | ((sign | R) & 0xFF);
        cl += c;
        return R;
    }
    ...
    private void PULS() {
        ...
        if ((m & 0x10) != 0) {
            X = (mem.read(S) << 8) | mem.read(S + 1);
            S += 2;
            cl += 2;
        }
        if ((m & 0x20) != 0) {
            Y = (mem.read(S) << 8) | mem.read(S + 1);
            S += 2;
            cl += 2;
        }
        if ((m & 0x40) != 0) {
            U = (mem.read(S) << 8) | mem.read(S + 1);
            S += 2;
            cl += 2;
        }
        if ((m & 0x80) != 0) {
            PC = (mem.read(S) << 8) | mem.read(S + 1);
            S += 2;
            cl += 2;
        }
        cl += 5;
    ...
    private void PULU() {
        ...
        if ((m & 0x10) != 0) {
            X = (mem.read(U) << 8) | mem.read(U + 1);
            U += 2;
            cl += 2;
        }
        if ((m & 0x20) != 0) {
            Y = (mem.read(U) << 8) | mem.read(U + 1);
            U += 2;
            cl += 2;
        }
        if ((m & 0x40) != 0) {
            S = (mem.read(U) << 8) | mem.read(U + 1);
            U += 2;
            cl += 2;
        }
        if ((m & 0x80) != 0) {
            PC = (mem.read(U) << 8) | mem.read(U + 1);
            U += 2;
            cl += 2;
        }
        cl += 5;
    ...
    private void CMP16(int R, int adr, int c) {
        int val = (mem.read(adr) << 8) | mem.read(adr + 1);
        m1 = R >> 8;
        m2 = (-val) >> 8;
...
//	getcc();
    ...
    private void BVS() {
        int m = mem.read(PC++);
        if ((((m1 ^ m2) & 0x80) == 0) && (((m1 ^ ovfl) & 0x80) != 0)) {
            PC += signedChar(m);
        }
        cl += 3;
    }
    ...
    private void Fetch() {
        ...
        sound_buffer[sound_addr] = (byte) mem.getSoundMem();
        sound_addr = (sound_addr + 1) % sound_size;
        if (sound_addr == 0) {
...
// Bxx
                    ...
                    default:
                        System.err.println("opcode 10 " + hex(opcode0x10, 2) + " not implemented");
            ...
            case 0x11:
                ...
                switch (opcode0x11) {
                    ...
                    default:
                        System.err.println("opcode 11" + hex(opcode0x11, 2) + " not implemented");
                        System.err.println(printState());
                } // of case opcode 0x11
                break;

            default:
                System.err.println("opcode " + hex(opcode, 2) + " not implemented");
                System.err.println(printState());
    ...
    private static String hex(int val, int size) {
    ...
    private static int signedChar(int v) {
        if ((v & 0x80) == 0) {
            return (v & 0xFF);
        }
    ...
    // force sign extension in a portable but ugly maneer
    private static int signed16bits(int v) {
        if ((v & 0x8000) == 0) {
            return (v & 0xFFFF);
        }
        int delta = -1; // delta is 0xFFFF.... independently of 32/64bits
    ...
    private static String r_tfr(int m) {
        String output = "";
        switch (m & 0xF0) {
            case 0x80:
                output += "A,";
    ...
    private static String r_pile(int m) {
        String output = "";
        if ((m & 0x80) != 0) {
            output += "PC,";
        }
    ...
    public String unassemble(int start, int maxLines) {
        ...
        for (int line = 0; line < maxLines; line++) {
            int mm = mem.read(where);
            where++;

            String output1 = hex(where - 1, 4) + '.';
